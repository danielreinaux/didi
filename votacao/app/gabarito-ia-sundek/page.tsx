...
// ── Critérios que a IA avalia na Sundek (1 selectbox por critério) ───────────
// Os tokens batem EXATAMENTE com a saída dos prompts (src/prompts/*.py Sundek)
// e com o que o build (src/build/gabarito_sundek.py) grava no campo "ia".
type Criterio = {
  key: string;
  label: string;
...
// "não se aplica" — pros critérios que o portão pulou (ex: bolso de um nao_short).
const NA = "n_a";
...
export default function GabaritoIASundek() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/gabarito_sundek.json").then((r) => {
    ...
    <div className="h-full overflow-y-auto">
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex flex-col gap-3">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <GabaritoNav atual="/gabarito-ia-sundek" />
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        {carregando ? (
          ...
          <p className={`text-center mt-20 text-sm ${TEXT_TERTIARY}`}>Não consegui carregar o gabarito_sundek.json.</p>
        ) : (
